...
\begin{equation}
m_e=\frac{\sum_{i=1}^m\sum_{j=1}^m\rho_{ij}^2}{m^2}=\bar{\rho}^2 (\#eq:Me2)
\end{equation}

It is an important parameter to describe the evolutionary process of a population of study.
...
\begin{equation}
4\frac{\color{red}{n_1}\color{blue}{n_2}}{n}\frac{(\color{red}{p_{1,l}}-\color{blue}{p_{2,l}})^2}{\bar{p}_l\bar{q}_l}=4n\omega_1 \omega_2 F_{st}^N=nF_{st}^W (\#eq:EG-chi)
\end{equation}
...
\end{equation}
in which 
- $\mathbf{U}$ is a $n\times n$ unitary matrix, corresponding to individual-level loading for each sample,
- $\mathbf{\Sigma}$ a $n\times m$ diagonal matrix, 
